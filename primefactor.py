...
try: 
    ...
    print("Numpy is not installed on your system. Please install it and try again.")
...
try:
    ...
    while i < len(primef):
        ...
        i += 1
        if i%1000 is 0: 
            print("Calculating "+str(i)+"th prime...")
    first = query%primef
    indices = numpy.where(first is 0)[0]
...
# Odd only
primef_o = primef/2.%1 # Needs to be a float
indices = primef_o.nonzero()
primef = primef[indices]
primef = primef[1:]
# Set the iterators
...
# Quick prime check
mods = query%primef
indices = numpy.where(mods is 0)[0]
primef = primef[indices]
...
while i < len(primef):
    ...
    if i%50 is 0: 
        ...
        if not deep: 
            ...
            counter=50
        if inum%counter is 0:
            print("Working ... "+str(inum)+" iterations... (currently checking "+str(primef[i])+")")
        #clean it up
        pslice = primef[:i]
        primer = input%pslice
...
npa = numpy.array(plist)
nmax = npa.max()
if nmax < 3 and nmax is not qbak: 
    plist.append(qbak)
elapsed=time.time()-starttime
if len(plist)>2: 
    print("The prime factorization for "+str(input)+" is "+str(plist))
else: 
